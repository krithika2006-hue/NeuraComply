...
### A. Measured Results (Active Implementation)

| Metric | Heuristic (Regex Baseline) | NeuraComply Semantic AI | Delta / Impact |
| :--- | :---: | :---: | :---: |
...
| **Human-in-the-Loop (HITL) Review Rate** | **0.0%** (Silent failure / unmapped drop) | **17.6%** (6 cases routed to triage) | Safe, measurable boundary |
| **Unmapped Noise Rejection** | Partial (regex false matches) | **100.0%** (Safely rejected) | Zero false positive security mappings |
| **Execution Latency per Line** | ~0.05 ms | ~0.50 ms | Sub-millisecond vector inference |

> [!NOTE]
> All figures above are directly generated by executing `npm run evaluate`. They are logged to `evaluation/evaluation_report.json`.
...
| **Embedding Engine** | 128-dimensional dense subword-semantic projection | Fine-tuned 384-dimensional domain-adapted transformer (`neura-minilm-secops-v1`) |
